...
export default function App() {
    ...
    if (view === 'dashboard' && facility) {
        return <AllocationDashboard userId={userId} facility={facility} patients={patients} setPatients={setPatients} toggleDarkMode={toggleDarkMode} darkMode={darkMode} />;
    }
...
// --- Component: Allocation Dashboard ---
const AllocationDashboard = ({ userId, facility, patients, setPatients, toggleDarkMode, darkMode }) => {
    ...
    const handleAllocation = async () => {
        ...
        try {
            const response = await fetch("http://127.0.0.1:8001/allocate", {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    patients: patients.map(p => ({ id: p.id, severity: p.triageTag === 'Immediate (Red)' ? 3 : p.triageTag === 'Delayed (Yellow)' ? 2 : 1, location: [0, 0] })),
            ...
            <div className="p-4 sm:p-8 bg-gray-50 dark:bg-gray-900">
                <div className="flex justify-center">
                    <button
                        ...
                        className="px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:bg-gray-500 font-semibold transition"
                    >
                ...
                {allocationResult.length > 0 && (
                    <div className="mt-6 mx-auto max-w-4xl bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
                        <h3 className="text-lg font-semibold mb-4 text-indigo-700 dark:text-indigo-300">
                            Allocation Results
                        </h3>
                        <ul className="space-y-2">
                            {allocationResult.map((r, i) => (
                                <li key={i} className="text-sm text-gray-700 dark:text-gray-200 p-3 bg-gray-100 dark:bg-gray-700 rounded">
                                    Patient <b>{r.patient_id.substring(0, 8)}...</b> → Hospital <b>{r.hospital_id}</b>{" "}
                                    (Distance: {r.distance}km, Severity: {r.severity})
                                </li>
                            ))}
                        </ul>
                    </div>
